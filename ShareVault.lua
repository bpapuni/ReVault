...
function ShareVaultMixin:OnShow()
	...
	-- ShareVaultExpirationWarningDialog:SetShown(C_WeeklyRewards.ShouldShowRetirementMessage() or C_WeeklyRewards.ShouldShowFinalRetirementMessage());
	self:FullRefresh();
end
...
function ShareVaultMixin:OnHide()
	...
	-- ShareVaultExpirationWarningDialog:Hide(); 
end

-- function ShareVaultMixin:OnEvent(event)
...
function ShareVaultMixin:IsReadOnly()
...
function ShareVaultMixin:Refresh(playSheenAnims)
	...
	-- if hasAvailableRewards then
		...
		for i, activity in ipairs(weeklyRewardsActivities) do
			...
			-- Check if activity.rewards is not nil and has at least one item
			if activity.rewards and #activity.rewards > 0 then
				local itemDBID = activity.rewards[1].itemDBID
				rewards[i] = { itemId = itemDBID }
			else
				-- Handle the case where activity.rewards is nil or empty
				rewards[i] = { itemId = {} }
			end
		end
		-- activities = rewards
		activities = weeklyRewardsActivities
	-- else
		-- activities = TEST_TABLE.Rewards;
	-- end
	for i, activityInfo in ipairs(activities) do
...
function ShareVaultMixin:UpdateTitle()
	-- local canClaimRewards = C_WeeklyRewards.CanClaimRewards();
...
-- function ShareVaultMixin:UpdatePreviousClaim()
...
-- function ShareVaultMixin:SelectActivity(activityFrame)
...
function ShareVaultMixin:TryDisplayingClassSetTutorial()
	local activities = C_WeeklyRewards.GetActivities();	
...
function ShareVaultMixin:FindFirstNonRaidActivityWithClassSetReward(activities)
	for _, activity in ipairs(activities) do
		if activity.type ~= Enum.WeeklyRewardChestThresholdType.Raid then
			for _, reward in ipairs(activity.rewards) do
				if reward.type == Enum.CachedRewardType.Item and not C_Item.IsItemKeystoneByID(reward.id) then
...
function ShareVaultMixin:ShowClassSetTutorial(parent)
	local helpTipInfo = {
...
function ShareVaultActivityMixin:Refresh(activityInfo)
	...
	if self.unlocked or self.hasRewards then
		...
		if self.hasRewards then
			self.ItemFrame:SetRewards(activityInfo.rewards[1].itemDBID);
			self.ItemGlow:Show();
...
function ShareVaultActivityMixin:OnSheenAnimFinished()
	self.RewardGenerated:Hide();
end
...
function ShareVaultActivityMixin:IsCompletedAtHeroicLevel()
...
-- function ShareVaultActivityMixin:OnMouseUp(button, upInside)
-- 	if button == "LeftButton" and upInside then
-- 		self:GetParent():SelectActivity(self);
-- 	end
-- end

function ShareVaultActivityMixin:CanShowPreviewItemTooltip()
...
function ShareVaultActivityMixin:HandlePreviewRaidRewardTooltip(itemLevel, upgradeItemLevel)
...
function ShareVaultActivityMixin:HandlePreviewMythicRewardTooltip(itemLevel, upgradeItemLevel, nextLevel)
...
function ShareVaultActivityMixin:HandlePreviewWorldRewardTooltip(itemLevel, upgradeItemLevel, nextLevel)
...
function ShareVaultActivityMixin:AddTopRunsToTooltip()
	GameTooltip_AddBlankLineToTooltip(GameTooltip);
...
function ShareVaultActivityMixin:HandlePreviewPvPRewardTooltip(itemLevel, upgradeItemLevel)
...
function ShareVaultActivityMixin:GetDisplayedItemDBID()
...
-- function ShareVaultConfirmSelectionMixin:OnEvent(event, ...)
-- 	self:RefreshRewards();
-- end

-- function ShareVaultConfirmSelectionMixin:ShowPopup(itemDBID, activityInfo)
...
-- function ShareVaultConfirmSelectionMixin:RefreshRewards()
-- 	local heightUsed = 19;
...
-- 		else
-- 			alsoItemsFrame.pool = CreateFramePool("FRAME", alsoItemsFrame, "ShareVaultAlsoItemTemplate");
-- 		end
-- 		for i, rewardInfo in ipairs(self.activityInfo.rewards) do
